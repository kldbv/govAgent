...
const okedCodes = [
  // Major OKED sections for Kazakhstan
...
const enhancedProgramsData = [
  // Update existing programs with BPM-aligned data
  {
    id: 1,
...
async function seedBPMData() {
  try {
    ...
    const existingRegions = await pool.query('SELECT COUNT(*) FROM regions');
    ...
    if (regionCount === 0) {
      ...
      for (const region of kazakhstanRegions) {
        await pool.query(
          `INSERT INTO regions (code, name_kz, name_ru, region_type)
           VALUES ($1, $2, $3, $4)`,
          [region.code, region.name_kz, region.name_ru, region.region_type || 'region']
        );
      }
    ...
    // Check if OKED codes already exist
    const existingOked = await pool.query('SELECT COUNT(*) FROM oked_codes');
    ...
    if (okedCount === 0) {
      ...
      for (const oked of okedCodes) {
        await pool.query(
          `INSERT INTO oked_codes (code, name_kz, name_ru, parent_code, level)
           VALUES ($1, $2, $3, $4, $5)`,
          [oked.code, oked.name_kz, oked.name_ru, oked.parent_code || null, oked.level]
        );
      }
    ...
    // Update the first program with enhanced data
