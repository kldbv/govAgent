...
interface DocumentGuidance {
  ...
  preparation_steps: GuidanceStep[];
...
export class GuidanceService {
  ...
  async generateDocumentGuidance(
    programId: number,
    userProfile: UserProfile
  ): Promise<DocumentGuidance> {
    // Get program details
    ...
    // Store guidance in database for caching
  ...
  private async generateAIGuidance(
    ...
    try {
      ...
      const guidance = JSON.parse(response) as DocumentGuidance;
  ...
  private async cacheGuidance(
    ...
    try {
      await pool.query(
        `INSERT INTO program_guidance (program_id, user_id, guidance_data, created_at, expires_at)
         VALUES ($1, $2, $3, NOW(), NOW() + INTERVAL '30 days')
         ON CONFLICT (program_id, user_id) 
         DO UPDATE SET guidance_data = EXCLUDED.guidance_data, 
                       created_at = NOW(),
                       expires_at = NOW() + INTERVAL '30 days'`,
        [programId, userId, JSON.stringify(guidance)]
      );
    } catch (error) {
      console.error('Error caching guidance:', error);
      // Don't throw - guidance should work even if caching fails
    }
  }

  async getCachedGuidance(
    programId: number,
    userId: number
    ...
    try {
      ...
      if (result.rows.length > 0) {
        return JSON.parse(result.rows[0].guidance_data) as DocumentGuidance;
      }
    } catch (error) {
  ...
  async trackGuidanceUsage(
    programId: number,
    userId: number,
    action: 'viewed' | 'downloaded_template' | 'completed_step'
  ): Promise<void> {
    try {
      await pool.query(
        `INSERT INTO guidance_usage (program_id, user_id, action, created_at)
         VALUES ($1, $2, $3, NOW())`,
        [programId, userId, action]
      );
    } catch (error) {
      console.error('Error tracking guidance usage:', error);
      // Don't throw - tracking should not break the main functionality
    }
  }
}
