...
async function ensureProgram(programId) {
    try {
        ...
        const existing = await database_1.default.query('SELECT id FROM business_programs WHERE id = $1', [programId]);
        if (existing.rows.length === 0) {
          ...
          NOW()
            ...
            await database_1.default.query(`SELECT setval(pg_get_serial_sequence('business_programs','id'), (SELECT GREATEST(MAX(id), 1) FROM business_programs))`);
            console.log('Inserted placeholder program and adjusted sequence.');
        }
        else {
            console.log('Program exists. Ensuring it is active...');
            await database_1.default.query('UPDATE business_programs SET is_active = true, updated_at = NOW() WHERE id = $1', [programId]);
...
exports.default = ensureProgram;
//# sourceMappingURL=ensureProgram.js.map