...
export default function SubsidiesPage() {
  ...
  const load = async (nextPage = 1) => {
    try {
      setLoading(true)
      const data = await searchPrograms({ program_type: 'субсидии', limit, page: nextPage, sort: 'newest', open_only: onlyOpen ? 1 : 0 } as any)
      const list = data.programs || data || []
      setPrograms(prev => nextPage === 1 ? list : [...prev, ...list])
      setHasMore(list.length === limit)
      setPage(nextPage)
    } catch (e: any) {
      setError(e.message || 'Не удалось загрузить субсидии')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900">Субсидии</h1>
        <p className="text-gray-600 mt-2">Программы субсидирования и компенсации затрат</p>
      </div>
      ...
      {loading ? (
        <div className="card p-8 text-center">
          ...
          <p className="text-gray-600">Загрузка субсидий...</p>
        </div>
      ) : error ? (
        <div className="card p-8 text-center">
        ...
        <div className="card p-8 text-center">
          <p className="text-gray-600">Пока нет программ субсидирования</p>
        </div>
      ) : (
        <>
